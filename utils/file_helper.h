...
namespace SmartNS {
    ...
    static std::string get_hostname_for_process(const std::string &file_path, size_t process_i) {
        std::string line = get_line_n(file_path, process_i);
    ...
    /// processes file. The autorun process file is formatted like so:
    /// <DNS name> <UDP port> <NUMA>
    static std::string get_udp_port_for_process(const std::string &file_path, size_t process_i) {
        std::string line = get_line_n(file_path, process_i);
    ...
    static std::string get_uri_for_process(const std::string &file_path, size_t process_i) {
        std::string hostname = get_hostname_for_process(file_path, process_i);
        std::string udp_port_str = get_udp_port_for_process(file_path, process_i);
