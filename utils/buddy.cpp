...
#define BUDDY_BLOCK_ORDER 12 /**< @brief Minimum block order */
...
int buddy_list_bucket::dump_buddy_table() {
    buddy_list *blt;
    buddy_list_bucket *blbt = this;
    ...
    while (1) {
        ...
        LIST_FOREACH(blt, &blbt->ptr, next_freelist) {
            ...
            if (blt->use == FREE)
                ...
                printf("    %11p%8s%16d\n", blt, "USED", blt->size);
            assert(next_power2(blt->size) == (1u << blbt->order));
        }
        printf("\n");
 ...
 */
int buddy_list_bucket::buddy_try_merge(buddy_list *blt) {
    int merge_count = 0;

    assert(sizeof(unsigned long) >= sizeof(buddy_list *));

    while (1) {
        unsigned long pointer_as_long = (unsigned long)blt;
        unsigned int size = blt->size + sizeof(buddy_list);
        buddy_list_bucket *blbt = reinterpret_cast<buddy_list_bucket *>(buddy_base_address);
        // Find the bucket we need
        while (size > (1u << blbt->order)) {
            blbt++;
        }
        ...
        buddy_list *possible_buddy = reinterpret_cast<buddy_list *> (pointer_as_long);
        if (possible_buddy->use == FREE &&
            possible_buddy->size == (size - sizeof(buddy_list))) {
            ...
            buddy_list *smallest_address = (blt < possible_buddy) ? blt : possible_buddy;
            smallest_address->size = 2 * size - sizeof(buddy_list);
 ...
 */
void buddy_list_bucket::buddy_free(void *ptr) {
    buddy_list *blt = (buddy_list *)ptr;
    blt--;
    ...
    if (blbt->count == 0) {
        blt->size = size - sizeof(buddy_list);
        blt->use = FREE;
        blbt->count++;
        LIST_INSERT_HEAD(&blbt->ptr, blt, next_freelist);
        memset(blt + 1, 0, blt->size);
        assert(blbt->count == static_cast<unsigned int>(initial_count + 1));
        return;
    }
    ...
    LIST_INSERT_HEAD(&blbt->ptr, blt, next_freelist);
    memset(blt + 1, 0, blt->size);
    assert(blbt->count == initial_count + 1u);
}
...
void buddy_list_bucket::buddy_split(buddy_list_bucket *bucket) {
    ...
    // Update the BLTs
    blt->use = FREE;
    blt->size = (1 << bucket->order) - sizeof(buddy_list);

    void *address = ((char *)blt) + (1 << bucket->order);
    buddy_list *new_blt = (buddy_list *)address;

    // new_blt->next_freelist = NULL;
    new_blt->use = FREE;
    new_blt->size = (1 << bucket->order) - sizeof(buddy_list);
...
void *buddy_list_bucket::buddy_alloc(uint64_t size) {
    ...
    buddy_list *blt = LIST_FIRST(&blbt->ptr);
    assert(blt && "LIST_FIRST returned NULL");
    LIST_REMOVE(blt, next_freelist);
    blt->use = USED;
    blbt->count--;
    ret = (char *)blt;
    ret += sizeof(buddy_list);
    return ret;
}
 ...
 */
buddy_list_bucket *buddy_list_bucket::create_buddy_table(size_t numa_node, unsigned int power_of_two) {
    int i;
    int size;
    int list_count;
    // void *memory;
    buddy_list_bucket *bsystem;
    ...
    bsystem = (buddy_list_bucket *)calloc(list_count + 1, sizeof(buddy_list_bucket));
    if (bsystem == NULL) {
        return NULL;
    }
    ...
    buddy_list *primordial = reinterpret_cast<buddy_list *>(bsystem->buddy_base_address);
    primordial->use = FREE;
    primordial->size = (1 << power_of_two) - sizeof(buddy_list);
