// Client: sudo ./rx_unlimited_working_set_in_cache -deviceName mlx5_2 -batch_size 1 -outstanding 32 -nodeType 0 -payload_size 8192 -threads 12 -nb_rxd 64 -nb_txd 64 -pkt_buf_size 8192 -pkt_handle_batch 1
// Relay:  sudo ./rx_unlimited_working_set_in_cache -deviceName mlx5_2 -batch_size 1 -outstanding 32 -nodeType 1 -payload_size 8192 -threads 12 -nb_rxd 64 -nb_txd 64 -pkt_buf_size 8192 -pkt_handle_batch 1
// Server: sudo ./rx_unlimited_working_set_in_cache -deviceName mlx5_0 -batch_size 1 -outstanding 32 -nodeType 2 -payload_size 8192 -threads 12 -serverIp 10.0.0.201 -nb_rxd 64 -nb_txd 64 -pkt_buf_size 8192 -pkt_handle_batch 1
...
void sub_task_relay(size_t thread_index, qp_handler *handler_server, vhca_resource *resource, void **bufs) {
    ...
    size_t local_mr_addr = reinterpret_cast<size_t>(handler_server->buf);
    size_t local_rkey = handler_server->mr->lkey;
    ...
    offset_handler send(NB_RXD, PKT_BUF_SIZE, NB_TXD * PKT_BUF_SIZE);
    offset_handler send_comp(NB_RXD, PKT_BUF_SIZE, NB_TXD * PKT_BUF_SIZE);

    size_t rx_depth = handler_server->rx_depth;

    for (size_t i = 0;i < rx_depth; i++) {
        handler_server->recv_sge_list[0].addr = recv.offset() + local_mr_addr;
        handler_server->recv_sge_list[0].length = PKT_BUF_SIZE;
        handler_server->recv_sge_list[0].lkey = local_rkey;
        handler_server->recv_wr->num_sge = 1;
        handler_server->recv_wr->sg_list = handler_server->recv_sge_list;
        handler_server->recv_wr->wr_id = recv.offset() + local_mr_addr;
        handler_server->recv_wr->next = nullptr;

        assert(ibv_post_recv(handler_server->qp, handler_server->recv_wr, &handler_server->recv_bar_wr) == 0);
        recv.step();
    }

    for (int i = 0;i < handler_server->num_wrs;i++) {
        handler_server->recv_sge_list[i].lkey = local_rkey;
        handler_server->recv_wr[i].sg_list = handler_server->recv_sge_list + i;
        handler_server->recv_wr[i].num_sge = 1;
        if (i != 0) {
            handler_server->recv_wr[i - 1].next = handler_server->recv_wr + i;
        }
    ...
    // CQ
    ibv_cq *sq_cq = create_dma_cq(resource->pd->context, 256);
    // QP
    ibv_qp *qp = create_dma_qp(resource->pd->context, resource->pd, sq_cq, sq_cq, 512);
    init_dma_qp(qp);
    ...
    ibv_cq *invalid_sq_cq = create_dma_cq(resource->pd->context, 256);
    ibv_qp *invalid_qp = create_dma_qp(resource->pd->context, resource->pd, invalid_sq_cq, invalid_sq_cq, 512);
    init_dma_qp(invalid_qp);
    dma_qp_self_connected(invalid_qp);
    ibv_qp_ex *invalid_dma_qpx = ibv_qp_to_qp_ex(invalid_qp);
    mlx5dv_qp_ex *invalid_dma_mqpx = mlx5dv_qp_ex_from_ibv_qp_ex(invalid_dma_qpx);
    invalid_dma_mqpx->wr_invcache_direct_init(invalid_dma_mqpx);

    // mkey
    uint32_t local_mr_mkey = handler_server->mr->lkey;
    ...
    while (!stop_flag && !done && !should_auto_exit(loop_begin_tsc)) {
        if ((recv.index() - recv_comp.index()) < (rx_depth - PKT_HANDLE_BATCH)) {
            for (size_t i = 0;i < PKT_HANDLE_BATCH;i++) {
                ...
                handler_server->recv_sge_list[i].length = PKT_BUF_SIZE;
                handler_server->recv_wr[i].wr_id = recv.offset() + local_mr_addr;
        ...
        int ne_recv = ibv_poll_cq(handler_server->recv_cq, CTX_POLL_BATCH, wc_recv);
        if (ne_recv != 0 && begin_time.tv_sec == 0) {
            clock_gettime(CLOCK_MONOTONIC, &begin_time);
        }

        int ne_dma_invalid = ibv_poll_cq(invalid_sq_cq, CTX_POLL_BATCH, wc_recv);
        for (int i = 0;i < ne_dma_invalid;i++) {
            assert(wc_recv[i].status == IBV_WC_SUCCESS);
        }

        int ne_dma = ibv_poll_cq(sq_cq, CTX_POLL_BATCH, wc_recv);
        for (int i = 0;i < ne_dma;i++) {
            assert(wc_recv[i].status == IBV_WC_SUCCESS);

            invalid_dma_qpx->wr_id = send_comp.index();
            invalid_dma_qpx->wr_flags = send_comp.index() % 32 == 31 ? IBV_SEND_SIGNALED : 0;
            invalid_dma_mqpx->wr_invcache_direct(invalid_dma_mqpx, local_mr_mkey, (uint64_t)handler_server->buf + send_comp.offset(), FLAGS_payload_size, false);
        ...
        for (int i = 0;i < ne_recv;i++) {
            ...
            while (send.index() - send_comp.index() > 200) {
                int ne_dma = ibv_poll_cq(sq_cq, CTX_POLL_BATCH, wc_recv);
                for (int i = 0;i < ne_dma;i++) {
                    assert(wc_recv[i].status == IBV_WC_SUCCESS);
            ...
            dma_qpx->wr_id = send.index();
            dma_qpx->wr_flags = IBV_SEND_SIGNALED;
            dma_mqpx->wr_memcpy_direct(dma_mqpx, remote_mr_mkey, (uint64_t)resource->addr + send.offset(), local_mr_mkey,
                (uint64_t)handler_server->buf + send.offset(), FLAGS_payload_size);
            send.step();
        }
...
void sub_task_client(size_t thread_index, qp_handler *qp_handler) {
    ...
    offset_handler send_client(NB_TXD, PKT_BUF_SIZE, 0);
    offset_handler send_client_comp(NB_TXD, PKT_BUF_SIZE, 0);

    size_t ne_send_client;
    ...
    while (!stop_flag && !should_auto_exit(loop_begin_tsc)) {
        ...
        for (size_t i = 0; i < ne_send_client; i++) {
            ...
            send_client_comp.step(PKT_HANDLE_BATCH);
        }

        if (send_client.index() - send_client_comp.index() <= PKT_SEND_OUTSTANDING - PKT_HANDLE_BATCH) {
            for (size_t i = 0;i < PKT_HANDLE_BATCH;i++) {
                qp_handler->send_sge_list[i].addr = send_client.offset() + reinterpret_cast<size_t>(qp_handler->buf);
                qp_handler->send_sge_list[i].length = FLAGS_payload_size;
                qp_handler->send_wr[i].wr_id = send_client.index();
                if (i == PKT_HANDLE_BATCH - 1) {
                    qp_handler->send_wr[i].next = nullptr;
...
void benchmark() {
    ...
    tcp_param net_param;
    net_param.isServer = FLAGS_nodeType == SERVER ? false : true;
    ...
    for (size_t i = 0; i < FLAGS_threads; i++) {
        ...
        if (FLAGS_nodeType == CLIENT) {
            ...
            threads[i] = std::thread(sub_task_client, now_index, qp_handlers_server[i]);
        } else if (FLAGS_nodeType == SERVER) {
            threads[i] = std::thread(sub_task_server, now_index, resources + i);
        } else {
    ...
    if (FLAGS_nodeType == RELAY) {
        printf("RESULT|experiment=2|method=rx_unlimited_working_set_in_cache|payload_size=%lu|threads=%lu|total_gbps=%.6f\n",
