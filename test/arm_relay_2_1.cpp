// Client: sudo ./arm_relay_2_1 -deviceName mlx5_2 -batch_size 1 -outstanding 32 -nodeType 0 -payload_size 2048 -threads 1
// Relay:  sudo ./arm_relay_2_1 -deviceName mlx5_2 -batch_size 1 -outstanding 32 -nodeType 1 -payload_size 2048 -threads 1
// Server: sudo ./arm_relay_2_1 -deviceName mlx5_0 -batch_size 1 -outstanding 32 -nodeType 2 -payload_size 2048 -threads 1  -serverIp 10.0.0.201
...
void sub_task_relay(size_t thread_index, qp_handler *handler_server, qp_handler *handler_client) {
    ...
    while (!stop_flag && !done && !should_auto_exit(loop_begin_tsc)) {
        ...
        for (int i = 0;i < ne_recv;i++) {
            ...
            while (send.index() - send_comp.index() > 512) {
                int ne_send = poll_send_cq(*handler_client, wc_recv);
                for (int i = 0; i < ne_send; i++) {
                    assert(wc_recv[i].status == IBV_WC_SUCCESS);
                    send_comp.step(1);
                }
            }
            post_send_batch(*handler_client, 1, send, FLAGS_payload_size);
        }
...
void benchmark() {
    ...
    for (size_t i = 0; i < FLAGS_threads; i++) {
        ...
        if (FLAGS_nodeType == CLIENT) {
            ...
            threads[i] = std::thread(sub_task_server, now_index, qp_handlers_client[i]);
        } else {
            init_wr_base_write(*qp_handlers_client[i]);
    ...
    if (FLAGS_nodeType == RELAY) {
        printf("RESULT|experiment=2|method=arm_relay_2_1|payload_size=%lu|threads=%lu|total_gbps=%.6f\n",
