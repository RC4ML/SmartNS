...
int devx_query_host_net_functions(struct ibv_context *context, struct devx_host_net_functions *funcs) {
    ...
    if (ret) {
        SMARTNS_ERROR("Failed to query host net functions");
        return DEVX_FAILED;
    }
    ...
    if (status) {
        SMARTNS_ERROR("Query host net functions returned with status 0x%x, syndrome = 0x%x",
...
int devx_query_hca_caps_ex(struct ibv_context *context, struct devx_hca_capabilities *caps, bool other_function, uint16_t function_id) {
    ...
    MLX5_SET(query_hca_cap_in, in, op_mod,
        static_cast<int>(PRIV_DOCA_MLX5_HCA_CAP_OP_MOD_GENERAL_DEVICE) | static_cast<int>(PRIV_DOCA_MLX5_HCA_CAP_OPMOD_GET_CUR));
    ...
    int ret = mlx5dv_devx_general_cmd(context, in, sizeof(in), out, sizeof(out));
    if (ret) {
        SMARTNS_ERROR("Failed to query HCA capabilities");
        return DEVX_FAILED;
    }

    /* Inspect the returned status of the command and print the syndrome in case of error. */
    if (MLX5_GET(query_hca_cap_out, out, status) != 0) {
        SMARTNS_ERROR("Query devx HCA capabilities failed, syndrome = 0x%x",
    ...
    if (caps->hca_cap_2) {
        ...
        MLX5_SET(query_hca_cap_in, in, opcode, MLX5_CMD_OP_QUERY_HCA_CAP);
        MLX5_SET(query_hca_cap_in, in, op_mod,
            static_cast<int>(PRIV_DOCA_MLX5_HCA_CAP_OP_MOD_GENERAL_DEVICE_CAP_2) | static_cast<int>(PRIV_DOCA_MLX5_HCA_CAP_OPMOD_GET_CUR));
        MLX5_SET(query_hca_cap_in, in, other_function, other_function);
        MLX5_SET(query_hca_cap_in, in, function_id, function_id);
        ...
        /* Inspect the returned status of the command and print the syndrome in case of error. */
        if (MLX5_GET(query_hca_cap_out, out, status) != 0) {
            SMARTNS_ERROR("Query devx HCA capabilities2 failed, syndrome = 0x%x",
                MLX5_GET(query_hca_cap_out, out, syndrome));
            return DEVX_FAILED;
        }
...
int devx_query_hca_vuid(struct ibv_context *context, uint16_t vhca_id, struct devx_vuid *vuid) {
    ...
    int ret = mlx5dv_devx_general_cmd(context, in, sizeof(in), out, sizeof(out));
    if (ret) {
        SMARTNS_ERROR("Failed to query vuid, vhca_id=%u", vhca_id);
        return DEVX_FAILED;
    }
    /* Inspect the returned status of the command and print the syndrome in case of error. */
    int status = MLX5_GET(query_vuid_out, out, status);
    if (status) {
        SMARTNS_ERROR("Query DevX VUID returned with status 0x%x", status);
        return DEVX_FAILED;
    }
    uint16_t nb_out_vuids = MLX5_GET(query_vuid_out, out, num_of_entries);
    if (nb_out_vuids != 1) {
        SMARTNS_ERROR("Query DevX VUID returned with nb_out_vuids %u", nb_out_vuids);
        return DEVX_FAILED;
    }
...
int devx_query_emulated_functions_info(struct ibv_context *context,
    ...
    if (ret) {
        SMARTNS_ERROR("Failed to query emulated functions info");
        return DEVX_FAILED;
    }
    nb_emulated_fns = MLX5_GET(query_emulated_functions_info_out, out, num_emulated_functions);
    if (nb_emulated_fns > nb_max_functions) {
        SMARTNS_ERROR("Query DevX emulated functions found more devices than requested, found %u, max %u", nb_emulated_fns, nb_max_functions);
        return DEVX_FAILED;
    }
    size_t emulated_info_size_bytes = MLX5_ST_SZ_BYTES(emulated_function_info);
    uint8_t *emulated_fn_info_out = (uint8_t *)MLX5_ADDR_OF(query_emulated_functions_info_out, out, emulated_function_info_array);
    uint16_t fn_idx;
